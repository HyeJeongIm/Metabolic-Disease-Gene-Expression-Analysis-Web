...
def create_search_bar():
    ...
    if not st.session_state['pressed']:
        ...
        valid_genes, error_message = check_gene_names(st.session_state['gene_list'], path)
        ...
        # "no specific group"이 아닌 경우에만 임계값 입력 칸을 보여줍니다.
        if group != 'no specific group':
            valid_threshold = get_threshold(threshold_key, group)  # 임계값 업데이트
...
#         valid_threshold = [default_threshold, group]  # 기본 임계값과 그룹 설정
#         if group != 'no specific group':
#             valid_threshold = get_threshold(threshold_key, group)  # 임계값과 그룹 업데이트
            
#             _, col2 = st.columns([8, 1])
#             with col2:
#                 apply_clicked = st.button('Apply')
        
#         # 임계값과 그룹을 사용하여 네트워크 다이어그램 표시
